...
anon_create()
{
	...
	mmobj_t *res = (mmobj_t*)slab_obj_alloc(anon_allocator);
	mmobj_init(res,&anon_mmobj_ops);
	res->mmo_refcount = 1;
	return res;
...
anon_put(mmobj_t *o)
{
		...
		list_iterate_begin(&(o->mmo_respages),tmp,pframe_t,pf_olink){
			/*if(pframe_is_pinned(tmp)){
				dbg(DBG_PRINT, "(GRADING3C36\n");
				pframe_unpin(tmp); 
			}*/
			/*if(pframe_is_dirty(tmp)){
				dbg(DBG_PRINT, "(GRADING3C37\n");
				 pframe_clean(tmp);
			}*/
			pframe_free(tmp);
		}list_iterate_end();
...
anon_fillpage(mmobj_t *o, pframe_t *pf)
{
		...
		pframe_t *p=pframe_get_resident(pf->pf_obj,pf->pf_pagenum);
        
       /* if(p == NULL) {
			dbg(DBG_PRINT, "(GRADING3C40\n");
			return 0;
		}*/

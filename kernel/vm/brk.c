...
do_brk(void *addr, void **ret)
{
    ...
    vmarea_t *tmp = vmmap_lookup(curproc->p_vmmap,ADDR_TO_PN(curproc->p_start_brk));
    /*if(tmp==NULL){
		dbg(DBG_PRINT, "(GRADING3C46\n");
        return -ENOMEM;
	}*/

    uint32_t old = ADDR_TO_PN((uint32_t)curproc->p_brk-1);
    uint32_t new = ADDR_TO_PN((uint32_t)addr-1);
    ...
    if(old==new){
        ...
        *ret = addr;
		...
		dbg(DBG_PRINT, "(GRADING3D)\n");
        if(vmmap_is_range_empty(curproc->p_vmmap,old+1,new-old)){ /* the boundary contains old*/
			dbg(DBG_PRINT, "(GRADING3D)\n");
            curproc->p_brk = addr;
            *ret = addr;
            tmp->vma_end = new+1;
		...
		dbg(DBG_PRINT, "(GRADING3D)\n");
        vmmap_remove(curproc->p_vmmap,new+1,old-new);
        curproc->p_brk = addr;
        *ret = addr;

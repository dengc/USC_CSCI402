...
sys_read(read_args_t *arg)
{
        ...
        /*if ((kern_args.buf = page_alloc()) == NULL) {
				dbg(DBG_PRINT, "(GRADING3C21\n");
                curthr->kt_errno = -err;
                return -1;
        }*/
        
        if ((read = do_read(kern_args.fd, kern_args.buf, kern_args.nbytes)) < 0) {
				dbg(DBG_PRINT, "(GRADING3D)\n");
        ...
        copy_to_user(arg->buf, kern_args.buf, read);
        
        /*if ((err = copy_to_user(arg->buf, kern_args.buf, read)) < 0) {
				dbg(DBG_PRINT, "(GRADING3C23\n");
                page_free(kern_args.buf);
                curthr->kt_errno = -err;
                return -1;
        }*/
...
sys_write(write_args_t *arg)
{
        ...
        /*if ((err = copy_from_user(kern_args.buf, arg->buf, arg->nbytes)) < 0) {
                dbg(DBG_PRINT, "(GRADING3C27\n");
                page_free(kern_args.buf);
                curthr->kt_errno = -err;
                return -1;
        }*/
        
        if ((write = do_write(kern_args.fd, kern_args.buf, kern_args.nbytes)) < 0) {
                dbg(DBG_PRINT, "(GRADING3D)\n");
                page_free(kern_args.buf);
...
{        
        ...
        /*if ((err = copy_from_user(&kern_args, arg, sizeof(kern_args))) < 0) {
				dbg(DBG_PRINT, "(GRADING3C30\n");
                curthr->kt_errno = -err;
                return -1;
        }*/
        
        dirent_t *temp = kmalloc(sizeof(dirent_t));
        uint32_t i = 0;
        int get = 0;
        dirent_t *pos = arg->dirp;
        while (i < kern_args.count / sizeof(dirent_t)) {
            get = do_getdent(kern_args.fd, temp);
            ...
            copy_to_user(pos, temp, get);
            
            /*if ((err = copy_to_user(pos, temp, get)) < 0) {
				dbg(DBG_PRINT, "(GRADING3C33\n");
                curthr->kt_errno = -err;
                return -1;
            }*/
            write+=get;
